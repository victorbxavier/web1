...
const server = http.createServer((req, res) => {
  ...
  // Verifique se a solicitação é POST e no caminho /enviar
    ...
    req.on('end', () => {
      ...
      const conteudo = {Nome: nome,  Sobrenome: sobrenome};
      const json = JSON.stringify(conteudo);

      fs.appendFile("nomes.json", json, (err)=>{
        if(err){
          console.log(err);
        }else{
          console.log("\nFile Contents of file after append:",//Colocar como array de objetos JSON
          fs.readFileSync("nomes.json", "utf8"));
        }
      });
      ...
      res.writeHead(200, { 'Content-Type': 'text/html' });
      res.write(`<p>Nome: ${nome} Sobrenome: ${sobrenome}</p>`);
      res.end();
    });
  } else {
